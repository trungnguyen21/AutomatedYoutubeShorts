...
import scripts.generate_subtitle as gs

def createBackground(audioPath, videoPath): #the path to the mp3 file, the path to the video FILES (folder)
    print("Creating background for the video.Gathering Audio and Video files...")
    audio = MP3.MP3(audioPath)
    voice = AudioFileClip(audioPath)
    print("Done gathering files")
    
    durationAudio = audio.info.length
    print("Getting video file: {}/{}".format(videoPath, random.choice(os.listdir(f'{videoPath}'))))
...
def finalize(videoPath, audio_path, title, exportPath='./'): #the path to video FILES, the path to the mp3 file, the title of the video, export path
    ...
    #get the transcript
    transcript = gs.save_transcript(audio_path, title, exportPath)
